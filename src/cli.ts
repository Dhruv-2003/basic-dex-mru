...
import { Action, PoolAmountResponse, ShareAmountResponse, EthAmountResponse, UsdcAmountResponse } from './cli-types.ts';
...
const actions = {
    checkBalance: async (): Promise<void> => {
        const account = sm?.state.balances.find(account => account.wallet === selectedWallet.address)
    ...
    withdraw: async (shares: number): Promise<void> => {
        ...
        const signature = await signMessage(selectedWallet, WithdrawLiquiditySchema, inputs);
    ...
    swapETHtoUSDC: async (ethAmount: number): Promise<void> => {
        ...
        const signature = await signMessage(selectedWallet, SwapTokenSchema, inputs);
        const swapAction = SwapTokenSchema.actionFrom({ inputs, signature, msgSender: selectedWallet.address })
    ...
    return inquirer.prompt([
        {
            ...
            choices: ['Check balance', 'Check Pool', 'Init pool', 'Supply', 'Withdraw', 'Swap ETH to USDC', 'Swap USDC to ETH', 'Switch account', 'Exit'],
        },
    ]);
};
    ...
    while (!exit) {
        if (!selectedAccount) {
            ...
            if (selectedAccount === "Account 1" || selectedAccount === "Account 2") {
                ...
                console.log(`You have selected: ${selectedWallet.address.slice(0, 12)}...`);
            }
        }
        ...
        if (action === 'Exit') {
            ...
            if (action === 'Check balance') {
                ...
                action === 'Init pool' ? await actions.initPool(ethAmount, usdcAmount) : actions.supply(ethAmount, usdcAmount);
